...
MSG_NO_LOG_DATA = "NO LOG DATA AVAILABLE (FILE NOT FOUND)"  # reconcile with frontend constant
MISCONFIGURATION = "MISCONFIGURATION"  # reconcile with frontend constant
MISCONFIGURATION_MAP = [[MISCONFIGURATION, MISCONFIGURATION, MISCONFIGURATION, MISCONFIGURATION]]
...
def login_view(request):
    if request.method == 'POST':
        ...
        if user is not None and user.is_superuser:  # type: ignore
            ...
            return render(request, 'log_lens/login.html', {'error_message': f'{username} is not a superuser'})
        else:
            return render(request, 'log_lens/login.html', {'error_message': 'Invalid credentials'})
    else:
...
def api_logfile(request, path: str) -> HttpResponse:
    """
...
def log_lens_view(request) -> HttpResponse:
    ...
    A logged in superuser is required.
    """
    context = {"log_sources": get_source_mappings(),
               "LOG_LENS_DEBUG": LOG_LENS_DEBUG}
...
def get_source_mappings() -> list[list[Any]]:
    ...
    try:
        ...
        for handler_name in settings.LOGGING['handlers']:
            ...
            is_file_handler = settings.LOGGING['handlers'][handler_name]['class'] in handlers
...
def get_backup_slots(path: str) -> list[list[str]]:
    ...
    pattern_date = re.compile(re.escape(original_filename) + r'\.(\d{4}-[A-Za-z0-9_-]+)$')
