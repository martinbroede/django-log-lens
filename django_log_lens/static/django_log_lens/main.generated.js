...
const INITIAL_LOG_SOURCE = { content: "", lines: "", timestamp: -1 };
...
const fileName = Alpine.store("ui").selectedLogSource;
if (!fileName || fileName === NONE_SELECTED) {
return;
}
Alpine.store("ui").isLoading = true;
let logSource = await getLogFile(fileName);
let lines = "";
const setState = () => {
Alpine.store("ui").logSource = { timestamp: Date.now(), content: logSource, lines };
...
const renderer = new LogRenderer(logSource, Alpine.store("ui").limitLinesTo);
const renderImmediately = renderer.totalRenderedLines <= LINE_THRESHOLD;
switch (logSource) {
case MISCONFIGURATION:
logSource = markAsError(MISCONFIGURATION + " - Please review your LOGGING settings<br/>in your settings.py file.");
setState();
return;
case MSG_NO_LOG_DATA:
logSource = markAsError(MSG_NO_LOG_DATA);
toast(MSG_NO_LOG_DATA, 3000, "error");
setState();
return;
case GENERIC_ERROR_MESSAGE:
logSource = markAsError(GENERIC_ERROR_MESSAGE);
toast(GENERIC_ERROR_MESSAGE_SHORT, 3000, "error");
setState();
return;
default:
lines = renderer.generateLineNumbers();
if (renderImmediately) {
logSource = renderer.renderFileContent();
invalidateSearchResults();
}
setState();
}
setTimeout(() => {
const highlightedContent = renderImmediately ? logSource : renderer.renderFileContent();
invalidateSearchResults();
Alpine.store("ui").errorCount = renderer.errorCounter;
Alpine.store("ui").logSource = { timestamp: Date.now(), content: highlightedContent, lines };
if (forceReload) {
...
renderFileContent() {
const formatted = this.lines.map(
(line, index) => this.colorizeLogLevels(this.renderLine(line, index + 1), index + 1) + "<br />"
);
...
line += this.createVsCodeLink(lineNumber, filename, lineNum);
...
spanClass = "warning";
contentId = `warning-${this.warningCounter++}`;
} else if (level >= 20) {
spanClass = "info";
} else if (level >= 10) {
spanClass = "debug";
}
line = `<span code-line="${lineNumber}" id="${lineId}">${line.replace(fullMatch, "")}</span>`;
...
const lines = logFileElem.querySelectorAll("span[code-line]");
