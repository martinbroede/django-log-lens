...
addWordBreaksInCodeBlocks();
syncSelectedLogSource();
mimicRouting();
initNavScrollBehaviour();
initArchiveTabBehaviour();
...
return; // Skip fetching if on the login page
}
...
if (!isFirstLoadedSource && shouldSwitchToNav && !ui.autoRefresh) {
ui.activeTab = "nav";
}
}
...
function setUpSSE() {
if (location.pathname.includes("/login")) {
return null; // Skip SSE setup if on the login page
}
...
function addWordBreaksInCodeBlocks() {
document.querySelectorAll("code").forEach((block) => {
block.innerHTML = addWordBreaks(block.innerHTML);
});
}
function addWordBreaks(text) {
return text.replaceAll(/([\/\-\.])(?!<wbr\/>)/g, "$1<wbr/>");
}
